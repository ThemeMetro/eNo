...
{if $loggedin}
	<li>
		<a href="#" data-toggle="dropdown" class="dropdown-toggle" id="accountNotifications">
			<i class="far fa-bell"></i>  {if count($clientAlerts) > 0}<span class="badge up badge-primary">{$clientAlerts|count}</span>{/if}
		</a>		
...
{if $languagechangeenabled && count($locales) > 1}
	<li>
		...
		<ul class="dropdown-menu dropdown-scroll dropdown-tasks auto auto-width">
			<li class="dropdown-header">
				<i class="fa fa-list"></i> {$activeLocale.localisedName}
			</li>
			<li id="langScroll">
				<ul class="list-unstyled">
